...
package com.castlemock.web.mock.websocket.stomp.model.project.service.adapter;
...
import com.castlemock.core.mock.websocket.stomp.model.project.service.message.input.*;
import com.castlemock.core.mock.websocket.stomp.model.project.service.message.output.*;
...
/**
 * The WebSocket stomp project service adapter is responsible for providing the basic functionality for all the
 * project services.
 ...
 */
@Service
public class WebSocketStompProjectServiceAdapter implements ProjectServiceAdapter<WebSocketStompProjectDto> {

    @Autowired
    private ServiceProcessor serviceProcessor;
    private static final WebSocketStompTypeIdentifier WEBSOCKET_STOMP_TYPE_IDENTIFIER = new WebSocketStompTypeIdentifier();
     ...
     */
    @Override
    public WebSocketStompProjectDto create(WebSocketStompProjectDto dto) {
        final CreateWebSocketStompProjectOutput output = serviceProcessor.process(new CreateWebSocketStompProjectInput(dto));
        return output.getWebSocketStompProject();
    }
    ...
    public void delete(String id) {
        serviceProcessor.process(new DeleteWebSocketStompProjectInput(id));
    }
     ...
     */
    @Override
    public WebSocketStompProjectDto update(String id, WebSocketStompProjectDto dto) {
        final UpdateWebSocketStompProjectOutput output = serviceProcessor.process(new UpdateWebSocketStompProjectInput(id, dto));
        return output.getUpdatedWebSocketStompProject();
    }
     ...
     */
    @Override
    public List<WebSocketStompProjectDto> readAll() {
        final ReadAllWebSocketStompProjectsOutput output = serviceProcessor.process(new ReadAllWebSocketStompProjectsInput());
        return output.getWebSocketStompProjects();
    }
     ...
     */
    @Override
    public WebSocketStompProjectDto read(String id) {
        final ReadWebSocketStompProjectOutput output = serviceProcessor.process(new ReadWebSocketStompProjectInput(id));
        return output.getWebSocketStompProject();
    }
     ...
     */
    @Override
    public WebSocketStompProjectDto convertType(ProjectDto parent) {
        return new WebSocketStompProjectDto(parent);
    }
    ...
    public String exportProject(String id) {
        final ExportWebSocketStompProjectOutput output = serviceProcessor.process(new ExportWebSocketStompProjectInput(id));
    ...
    public void importProject(String projectRaw) {
        serviceProcessor.process(new ImportWebSocketStompProjectInput(projectRaw));
    }
    ...
    public List<SearchResult> search(SearchQuery searchQuery) {
        final SearchWebSocketStompProjectOutput output = serviceProcessor.process(new SearchWebSocketStompProjectInput(searchQuery));
