...
public class WebSocketBroadcasterController extends AbstractWebSocketViewController {

    private static final String PAGE = "mock/websocket/broadcaster/webSocketBroadcaster";
    private static final Logger LOGGER = Logger.getLogger(WebSocketBroadcasterController.class);
    ...
    @RequestMapping(value = "/{webSocketProjectId}/topic/{webSocketTopicId}/broadcaster/{webSocketBroadcasterId}", method = RequestMethod.GET)
    public ModelAndView defaultPage(@PathVariable final String webSocketProjectId, @PathVariable final String webSocketTopicId, @PathVariable final String webSocketBroadcasterId, final ServletRequest request) {
        ...
        final WebSocketBroadcasterDto webSocketBroadcaster = output.getWebSocketBroadcaster();

        final String protocol = getProtocol(request);
        //final String invokeAddress = getWebSocketInvokeAddress(protocol, request.getServerPort(), webSocketProjectId, webSocketTopicId, webSocketBroadcaster.getUri());
        final ModelAndView model = createPartialModelAndView(PAGE);
        model.addObject(WEBSOCKET_PROJECT_ID, webSocketProjectId);
        model.addObject(WEBSOCKET_TOPIC_ID, webSocketTopicId);
        model.addObject(WEBSOCKET_BROADCASTER, webSocketBroadcaster);
        return model;
    }
