...
public class ReadWebSocketBroadcasterService extends AbstractWebSocketProjectService implements Service<ReadWebSocketBroadcasterInput, ReadWebSocketBroadcasterOutput> {
     ...
     */
    @Override
    public ServiceResult<ReadWebSocketBroadcasterOutput> process(final ServiceTask<ReadWebSocketBroadcasterInput> serviceTask) {
        final ReadWebSocketBroadcasterInput input = serviceTask.getInput();
        final WebSocketBroadcasterDto webSocketBroadcasterDto = repository.findWebSocketBroadcaster(
                input.getWebSocketProjectId(), input.getWebSocketTopicId(), input.getWebSocketBroadcasterId());
        return createServiceResult(new ReadWebSocketBroadcasterOutput(webSocketBroadcasterDto));
    }
}
