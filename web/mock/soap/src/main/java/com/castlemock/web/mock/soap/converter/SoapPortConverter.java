...
public class SoapPortConverter {
  ...
  private Set<String> getImports(final String url,
      final WSDLDocument wsdlDocument) {
    final Document document = wsdlDocument.getDocument();
    final List<Element> importElements = DocumentUtility
        .getElements(document, WSDL_NAMESPACE, IMPORT_NAMESPACE);
  ...
  /**
   * THe method provides the functionality to parse a document and extract all the SOAP ports from
   * the SOAP ports. The method will also generate mocked responses if the {@code generateResponse}
   *
   * @param document The document which will be parsed
   * @param generateResponse Boolean value determining if a response should be generated for each
   * extracted operation.
  ...
  private SoapOperation createSoapOperation(final BindingOperation bindingOperation,
    ...
    final Message inputMessage = messages.stream()
        .filter(message -> portTypeOperation.getInput().getMessage().getLocalName()
    ...
    final Message outputMessage = messages.stream()
        .filter(message -> portTypeOperation.getOutput().getMessage().getLocalName()
            .equals(message.getName()))
        .findFirst()
        .orElseThrow((() -> new IllegalArgumentException("Unable to find the input message")));
    ...
    final MessagePart inputMessagePart = bindingOperation.getInput().getBody()
        ...
        .orElse(inputMessage.getParts().stream()
            .findFirst()
            .orElseThrow(
                () -> new IllegalArgumentException("Unable to find any input message part")));
    ...
    final MessagePart outputMessagePart = getMessageOutputMessagePart(bindingOperation,
        outputMessage);

    if (outputMessagePart != null) {
      SoapOperationIdentifier operationResponseIdentifier =
  ...
  private MessagePart getMessageOutputMessagePart(BindingOperation bindingOperation,
      Message outputMessage) {
    return bindingOperation.getOutput().getBody()
  ...
  private static SoapOperationIdentifier createSoapOperationIdentifier(
   ...
   */
  private static String generateDefaultBody(
